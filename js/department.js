...
function viewDeparment(element) {
    ...
    for (var i = 0; i < allLinks.length; i++) {
        if (allLinks[i] !== element) {
            allLinks[i].classList.remove("active-header");
        }
    }
}
...
function pagination(result) {
    ...
    $('#pagination-department').html(`<a title="Previous" href = "javascript:void(0)" onclick="toDepartmentPage(${activeNumber > 1 ? activeNumber - 1 : activeNumber})"><i class="fa-solid fa-chevron-left"></i></a>`);
    ...
    $('#pagination-department').append(`<a href = "javascript:void(0)" title="Next" onclick="toDepartmentPage(${activeNumber < result.totalPages ? activeNumber + 1 : activeNumber})"><i class="fa-solid fa-chevron-right"></i></a>`);
}
...
function initDapartmentTypeList() {
    $('.form-select-department').empty();
    $('.form-select-department').html(`<option value="">--Chọn nhóm phòng ban--</option>`);
